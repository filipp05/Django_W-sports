...
class CartTest(TestCase):
    def setUp(self) -> None:
        ...
        categories = []
        for _ in range(5):
            categories.append(Category.objects.create(name="test_category"))
        ...
        for i in range(10):
            ...
            product_attributes = Category.objects.filter(id__in=[category.id for category in product_categories]).values_list("productattribute", flat=True).filter(id__in=[attr.id for attr in attributes])
            product_attribute_values = []
            for attr in ProductAttribute.objects.filter(id__in=product_attributes):
                product_attribute_values.append(ProductAttributeValue(product=self.products[i], attribute=attr, int_value=random.randint(1, 10)))
            ...
            variant_attributes = Category.objects.filter(id__in=[category.id for category in product_categories]).values_list("productattribute", flat=True).filter(id__in=[attr.id for attr in variant_attributes])
            for attr in ProductAttribute.objects.filter(id__in=variant_attributes):
                variant_attribute_values.append(VariantsAttributeValue(product_variant=variant, attribute=attr, int_value=random.randint(1, 20)))
    ...
    def test_check_add_product(self):
        ...
        variant_attribute_values = product_variant.attribute_values.all()
        data = {}
        cart = Cart.objects.get(owner=self.owner)

        for value in variant_attribute_values:
            data[f"attribute-{value.attribute.id}"] = value.int_value

        self.client.post(f"/add_to_cart/{product.id}", data=data,)
        self.client.post